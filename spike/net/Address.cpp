...
namespace lim_webserver
{
    ...
    std::string IPv4Address::toString() const
    {
        ...
        // 获取端口号
        uint16_t port = htons(m_addr.sin_port);

        // 预留足够的空间来存储 IP 地址和端口号
        char resultStr[INET_ADDRSTRLEN + 6]; // IPv4地址最长为15个字符，端口号最长为5个字符

        // 使用 snprintf 构造字符串
        snprintf(resultStr, sizeof(resultStr), "%s:%u", ipv4Str, port);
    ...
    std::string IPv6Address::toString() const
    {
        ...
        inet_ntop(AF_INET, &(m_addr.sin6_addr), ipv6Str, INET6_ADDRSTRLEN);

        // 获取端口号
        uint16_t port = htons(m_addr.sin6_port);

        // 预留足够的空间来存储 IP 地址和端口号
        char resultStr[INET6_ADDRSTRLEN + 8]; // IPv6地址最长为39个字符，端口号最长为5个字符，方括号2个字符

        // 使用 snprintf 构造字符串
        snprintf(resultStr, sizeof(resultStr), "[%s]:%u", ipv6Str, port);
    ...
    std::string UnixAddress::toString() const
    {
        ...
        {
            char buffer[m_length - offsetof(sockaddr_un, sun_path) + 2]; // +2 for '\0' and '\\'
            snprintf(buffer, sizeof(buffer), "\\0%s", m_addr.sun_path + 1);
    ...
    {
        char buffer[100]; // 调整为足够大的缓冲区
        snprintf(buffer, sizeof(buffer), "[UnknownAddress family=%u]", m_addr.sa_family);
        return std::string(buffer);
    }
}