...
namespace lim_webserver
{
    namespace http
    {
        ...
        static ConfigerVar<uint64_t>::ptr g_http_request_buffer_size =
            Configer::Lookup("http.request.buffer_size", (uint64_t)(4 * 1024), "http request buffer size");

        static ConfigerVar<uint64_t>::ptr g_http_request_max_body_size =
            Configer::Lookup("http.request.max_body_size", (uint64_t)(64 * 1024 * 1024), "http request max body size");

        static ConfigerVar<uint64_t>::ptr g_http_response_buffer_size =
            Configer::Lookup("http.response.buffer_size", (uint64_t)(4 * 1024), "http response buffer size");

        static ConfigerVar<uint64_t>::ptr g_http_response_max_body_size =
            Configer::Lookup("http.response.max_body_size", (uint64_t)(64 * 1024 * 1024), "http response max body size");
        ...
        namespace
        {
            struct _RequestSizeIniter
            {
                _RequestSizeIniter()
                {
                    ...
                    g_http_request_buffer_size->addListener([](const uint64_t &ov, const uint64_t &nv) { s_http_request_buffer_size = nv; });
                    g_http_request_max_body_size->addListener([](const uint64_t &ov, const uint64_t &nv) { s_http_request_max_body_size = nv; });
                    g_http_response_buffer_size->addListener([](const uint64_t &ov, const uint64_t &nv) { s_http_response_buffer_size = nv; });
                    g_http_response_max_body_size->addListener([](const uint64_t &ov, const uint64_t &nv) { s_http_response_max_body_size = nv; });
                }
            };
        ...
        {
            HttpRequestParser *parser = static_cast<HttpRequestParser *>(data);
            parser->data()->setFragment(std::string(at, length));
        }

        void on_request_path(void *data, const char *at, size_t length)
        {
            HttpRequestParser *parser = static_cast<HttpRequestParser *>(data);
            parser->data()->setPath(std::string(at, length));
        }

        void on_request_query(void *data, const char *at, size_t length)
        {
            HttpRequestParser *parser = static_cast<HttpRequestParser *>(data);
            parser->data()->setQuery(std::string(at, length));
        }
        ...
        void on_request_http_field(void *data, const char *field, size_t flen, const char *value, size_t vlen)
        {
            ...
            parser->data()->setHeader(std::string(field, flen), std::string(value, vlen));
        }
