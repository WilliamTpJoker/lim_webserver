...
template <typename T> class ConcreteReclaimer;
...
public:
    ConcurrentQueue() : head_(new Node), tail_(head_.load(std::memory_order_relaxed)), size_(0) {}
...
private:
    ...
    void AcquireSafeNodeAndNext(std::atomic<Node *> &atomic_node, Node **node_ptr, Node **next_ptr, HazardPointer &node_hp, HazardPointer &next_hp);
    ...
    {
        template <typename... Args> RegularNode(Args &&...args) : value(std::forward<Args>(args)...) {}
...
template <typename T> Reclaimer::HazardPointerList ConcurrentQueue<T>::global_hp_list_;

template <typename T> class ConcreteReclaimer : public Reclaimer
{
...
private:
    ...
    {
        thread_local static ConcreteReclaimer reclaimer(ConcurrentQueue<T>::global_hp_list_);
        return reclaimer;
    }
};

template <typename T>
void ConcurrentQueue<T>::AcquireSafeNodeAndNext(std::atomic<Node *> &atomic_node, Node **node_ptr, Node **next_ptr, HazardPointer &node_hp,
