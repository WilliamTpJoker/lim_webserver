...
namespace lim_webserver
{
    ...
    static thread_local Sched *t_Sched = nullptr;
    static thread_local Fiber *t_fiber = nullptr;

    Sched::Sched(size_t threads, bool use_caller, const std::string &name)
        : m_name(name)
    {
        ASSERT(threads > 0);
        if (use_caller)
    ...
    void Sched::run()
    {
        ...
        while (true)
        {
            ...
            else if (ft.callback)
            {
                ...
                {
                    // 如果状态为READY，表明该回调协程通过YeildToReady返回Sched，将其重新规划入队列，回调协程则指定为新的协程
                    schedule(cb_fiber);
                    cb_fiber.reset();
                }
